...
<Figure src='vtracer/PipelineShowcase/Original.png' text="Sample Simplified 'S' Shape" />
...
<Figure src='vtracer/PipelineShowcase/LengthThresholds.svg' text='Length Thresholds 3.5/5.0/7.5' />

Introducing iterations, you can see more points are generated by subdivision:

<Figure src='vtracer/PipelineShowcase/MaxIterations.svg' text='Iterations 0/1/2' />

Note that VTracer's subdivision smoothing algorithm has a preset maximum number of iterations of 10, but terminates adaptively when no further smoothing can be done on the path.

Although the overall path is a nice approximation of the desired shape, we notice that some generated points are out-of-place. In this example, it is clear that one point at the bottom-right corner is off. We believe this is caused by segments near turning points having a dramatic change in length. Therefore, we further impose a restriction on the ratio of adjacent segments.

<Figure src='vtracer/PipelineShowcase/CheckRatio.png' text='A segment is only subdivided if it is longer than the length threshold and its neighbors are shorter than its double' />
...
<Figure src='vtracer/PipelineShowcase/Curve-fitted.png' text='At some turning points, handles of the Bezier curves are crossing each other, resulting in small corners which make the spline look rumpled.' />

### Detecting Crossing Handles

Given two line segments, we can check whether they cross each other by analyzing the signed angle differences at the segment endpoints. Denote the two line segments by AB and CD, we have:

<Figure src='vtracer/PipelineShowcase/Handles.svg' text='Left: Example non-crossing handles. Right: Example crossing handles' />

If we connect AD and BC, crossing handles can be detected by checking the signs of ∠DAB and ∠ABC. If their signs are the same, the two handles do not cross each other, vice versa.

<Figure src='vtracer/PipelineShowcase/DetectCrossingHandles.svg' text='Left: The signs of ∠DAB and ∠ABC are the same. Right: The signs are different.' />

### Retracting Handles

As we know that rumpledness is caused by crossing handles, we need to find a way to remove them (i.e. retracting them so that they do not cross each other anymore). At first, we tried the naïve way of cutting all the crossing handles by half, but it turned out to be too aggressive.

<Figure src='vtracer/PipelineShowcase/RetractHandlesByHalf.png' text='Result after naïvely cutting all crossing handles by half' />

Eventually we decided to retract the crossing handles to where they intersect.

<Figure src='vtracer/PipelineShowcase/RetractHandlesToIntersection.svg' text='Left: Before retracting crossing handles. Right: After retracting them to intersection. Each curve becomes smooth.' />
